...
│   └── deploy-frontend.yml           # Frontend build and Vercel deployment
...
8. **Verification and release**: Runs on-chain reads and local verification. The frontend is built and deployed separately by `.github/workflows/deploy-frontend.yml` using the environment configured in Vercel.
...
The frontend GitHub Actions workflow requires only `VERCEL_TOKEN`, `VERCEL_ORG_ID`, and `VERCEL_PROJECT_ID`. It pulls the linked Vercel project's production settings and environment variables before building and deploying. Configure the generated contract addresses and application secrets in Vercel Project Settings; this workflow does not build, upload, or deploy contracts.
