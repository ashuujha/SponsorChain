...
# Inspect the generated deployment record
cat deployment.json
```
...
A push to `main` runs `.github/workflows/deploy-contracts.yml`. It checks the Rust and Node code, builds and audits both WASM files, deploys fresh Testnet instances, initializes and links them, verifies RPC responses, runs integration tests with the generated IDs, builds the frontend, and publishes a deployment artifact. The frontend job consumes that artifact; it does not read contract IDs from independent secrets.

Required repository secrets are `STELLAR_ACCOUNT_SECRET`, `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`, `NEXTAUTH_SECRET`, `NEXTAUTH_URL`, `GITHUB_CLIENT_ID`, and `GITHUB_CLIENT_SECRET`. Contract IDs are generated by deployment and are never manually copied or stored as secrets.

The workflow produces `deployment.json`, `deployment.env`, `.env.local`, and `CONTRACTS.md`. `deployment.json` is the machine-readable source of truth. For local Testnet deployment, fund the deployer with Friendbot when needed:

```bash
curl -s "https://friendbot.stellar.org/?addr=$(stellar keys address sponsorchain-deployer)"
```

Fresh redeployment intentionally creates new addresses and synchronizes every consumer. For an in-place rollback, use the previously audited WASM hash and the authorized contract upgrade entrypoint; a fresh deployment rollback means redeploying the previous commit and consuming its artifact.
