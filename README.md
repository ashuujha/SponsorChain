...
A push to `main` runs `.github/workflows/deploy-frontend.yml`. It links the repository to the configured Vercel project, pulls its production project settings and environment variables, builds the Next.js frontend, and deploys the prebuilt frontend to Vercel. Contract deployment is intentionally separate and is not performed by this workflow.

Required repository secrets for the frontend workflow are `VERCEL_TOKEN`, `VERCEL_ORG_ID`, and `VERCEL_PROJECT_ID`. Configure the frontend's production environment variables—including the generated contract addresses, `NEXTAUTH_*`, and `GITHUB_*` values—in Vercel Project Settings. Contract deployment remains a separate local/manual operation.

Contract deployment produces `deployment.json`, `deployment.env`, `.env.local`, and `CONTRACTS.md`; `deployment.json` is the machine-readable source of truth. The frontend workflow only deploys the Vercel project. For local Testnet deployment, fund the deployer with Friendbot when needed:
